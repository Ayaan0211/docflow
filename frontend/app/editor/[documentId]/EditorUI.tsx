...
export default function EditorUI({
  ...
  return (
    <div ref={containerRef} className={`flex flex-col ${isFullscreen ? "h-screen" : "min-h-screen"} bg-gray-50`}>
      {/* Header */}
      <div className="bg-gray-900 text-white flex items-center justify-between px-6 py-4 shadow-lg">
        <div className="flex items-center gap-2">
          <button onClick={handleBackToDashboard} className="px-4 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-700 rounded-md transition">
            ← Back
          </button>
          <button 
          ...
          {lastSaved && <p className="text-sm text-gray-300">Saved at {lastSaved.toLocaleTimeString()}</p>}
        </div>
          ...
          {isEditingTitle ? (
            <>
              <input
                ...
                autoFocus
              />
              <button onClick={handleTitleSave} className="px-3 py-1 text-sm bg-blue-600 hover:bg-blue-700 rounded transition">
                Save
              </button>
              <button onClick={() => setIsEditingTitle(false)} className="px-3 py-1 text-sm bg-gray-700 hover:bg-gray-600 rounded transition">
                Cancel
              </button>
            </>
          ) : (
            <>
              ...
              <button onClick={() => setIsEditingTitle(true)} className="px-2 py-1 text-xs hover:bg-gray-800 rounded transition">
                ✏️
              </button>
            </>
          )}
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleUndo} className="px-3 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-700 rounded-md transition" title="Undo (Ctrl+Z)">
            ↶
          </button>
          <button onClick={handleRedo} className="px-3 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-700 rounded-md transition" title="Redo (Ctrl+Y)">
            ↷
          </button>
          <button onClick={() => setShowSearch(!showSearch)} className="px-3 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-700 rounded-md transition" title="Find (Ctrl+F)">
            🔍
          </button>
          <button onClick={toggleFullscreen} className="px-3 py-2 text-sm font-medium bg-gray-800 hover:bg-gray-700 rounded-md transition">
            {isFullscreen ? "⊗" : "⛶"}
          </button>
        </div>
      </div>
      ...
      {showSearch && (
        <div className="bg-gray-900 text-white flex items-center gap-3 px-6 py-3">
          <input
            ...
            autoFocus
          />
          <button onClick={handleSearch} className="px-3 py-1 text-sm bg-gray-800 hover:bg-gray-700 rounded-md">
            Search
          </button>
          <button onClick={prevMatch} className="px-2 py-1 text-sm bg-gray-800 hover:bg-gray-700 rounded-md">
            ↑
          </button>
          <button onClick={nextMatch} className="px-2 py-1 text-sm bg-gray-800 hover:bg-gray-700 rounded-md">
            ↓
          </button>
      ...
      {showTablePicker && (
        <div className="bg-gray-900 text-white flex items-center gap-3 px-6 py-3">
          ...
          <input
            ...
            placeholder="Cols"
          />
          <button onClick={insertTable} className="px-3 py-1 text-sm bg-gray-800 hover:bg-gray-700 rounded-md">
            Insert
          </button>
          <button onClick={() => setShowTablePicker(false)} className="px-2 py-1 text-sm bg-gray-800 hover:bg-gray-700 rounded-md">
            ✕
          </button>
        </div>
      )}
          ...
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-2xl w-full">
            ...
            <div className="mb-4 p-3 bg-blue-50 rounded">
              ...
              <ul className="text-xs text-gray-600 space-y-1">
                <li>• Fraction: <code className="bg-white px-1">\frac{"{numerator}"}{"{denominator}"}</code></li>
                <li>• Square root: <code className="bg-white px-1">\sqrt{"{x}"}</code></li>
                <li>• Superscript: <code className="bg-white px-1">x^{"{2}"}</code></li>
                <li>• Subscript: <code className="bg-white px-1">x_{"{1}"}</code></li>
                <li>• Sum: <code className="bg-white px-1">\sum_{"{i=1}"}^{"{n}"}</code></li>
                <li>• Integral: <code className="bg-white px-1">\int_{"{a}"}^{"{b}"}</code></li>
              </ul>
            </div>
